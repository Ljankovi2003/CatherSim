...
flowdatapath="/home/tingtaoz/data/storage/catheter/flowrandomsampling0819/"
outpath = "/home/tingtaoz/storage/traindatalevy/allparams/0819/"


############ simulation

batch = parse(Int64,ARGS[1])
print("batch ",batch)
# Ndata = 1000
Nbatch = 10
...
params = data=readdlm(string(flowdatapath,"params.txt"),skipstart=1,',')
ell_arr = params[:,1]
x2_arr = params[:,2]
x3_arr = params[:,3]
h_arr = params[:,4]
...
for ii=(1+Nbatch*(batch-1)):Nbatch*batch
    ell = ell_arr[ii]
    Lx = ell
    x2 = x2_arr[ii]
    x3 = x3_arr[ii]
    ...
    x50 = x20 = x2
    x40 = x30 = x3
    for flowfactor = 0.3333:0.3333:1
        for alpha = [1.5]
            y2 = h-Ly/2
            y5 = Ly/2-h
            p1 = (x10, y1)
            ...
            fprefix = @sprintf("sample%d_U0%guf%galpha%gtauR%g_%g_%g_%g",ii,U0, uf,alpha, tauR,x2,x3,h)
